...
gen treatment = 0
replace treatment = 1 if statefip == 6 & year >= 2012
replace treatment = 1 if statefip == 53 & year >= 2012
replace treatment = 1 if statefip == 2 & year >= 2014
replace treatment = 1 if statefip == 41 & year >= 2014
replace treatment = 1 if statefip == 5 & year >= 2016
replace treatment = 1 if statefip == 32 & year >= 2016
replace treatment = 1 if statefip == 23 & year >= 2016
replace treatment = 1 if statefip == 25 & year >= 2016
//replace treatment = 1 if statefip == 50 & year >= 2018
//replace treatment = 1 if statefip == 26 & year >= 2018

//reg employed treatment


gen sexid = 0
replace sexid=1 if sex == 1
replace sexid=0 if sex == 2

gen maleeffect = 0
replace maleeffect = 1 if sexid == 1 & treatment ==1

estpost tabstat employed if treatment == 1, by (sex) statistics(mean sd sem va)

gen agegroup1 = 0
replace agegroup1 = 1 if age < 29 

gen agegroup2 = 0
replace agegroup2 = 1 if age >= 30 & age <49

gen agegroup3 = 0
replace agegroup3 = 1 if age >= 50

gen group1effect = 0
replace group1effect = 1 if treatment == 1 & agegroup1 == 1

gen group2effect = 0
replace group2effect = 1 if treatment == 1 & agegroup2 == 1

gen group3effect = 0
replace group3effect = 1 if treatment == 1 & agegroup3 == 1

gen old = 0
replace old = 1 if agegroup1 == 0

gen youngeffect = 0
replace youngeffect = 1 if group1effect == 1

gen oldeffect = 0
replace oldeffect = 1 if treatment==1 & old == 1

// Regressions
ssc install reghdfe
ssc install ftools 

reghdfe employed treatment [pweight=perwt], absorb(statefip year) 
estimates store AA

reghdfe employed treatment, absorb(statefip age year)
estimates store AB

reghdfe employed treatment, absorb(statefip age year race)
estimates store AC

reghdfe employed treatment, absorb(statefip age year race metro)
estimates store AD

reghdfe employed treatment, absorb(statefip age year educ race metro)
estimates store AE

esttab AA AB AC AD AE, mtitle ("Model 1" "Model 2" "Model 3" "Model 4" "Model 5") nonum nocon

// effect of Age group
reghdfe employed treatment [pweight=perwt], absorb(statefip year educ race metro)
estimates store BA

reghdfe employed treatment if agegroup1==1 [pweight=perwt], absorb(statefip year educ race metro)
estimates store BB

reghdfe employed treatment if agegroup1==0 [pweight=perwt], absorb(statefip year educ race metro)
estimates store BC

reghdfe employed treatment youngeffect [pweight=perwt], absorb(statefip year educ race metro)
estimates store BD

esttab BA BB BC BD, mtitle ("Basic DD" "Young" "Old" "Young Effect") nonum nocon


//reghdfe employed treatment group1effect group2effect, absorb(statefip age year)
//estimates store CC

//reghdfe employed treatment group1effect group2effect group3effect, absorb(statefip age year)
//estimates store DD
...
gen urbaneffect = 0
replace urbaneffect = 1 if treatment==1 & urban == 1

reghdfe employed treatment, absorb(statefip age year educ race )
estimates store CA
reghdfe employed treatment if urban==0, absorb (state age year educ race )
estimates store CB
reghdfe employed treatment if urban==1, absorb (state age year educ race )
estimates store CC
reghdfe employed treatment urbaneffect, absorb (state age year educ race )
estimates store CD

esttab CA CB CC CD, mtitle ("Basic DD" "Rural" "Urban" "Urban Effect") nonum nocon
